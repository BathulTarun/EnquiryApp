...
const EnquiryDetailPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const engs2 = await OperatorService.getEngineerById(
        enqs2.assignedEngineerId!,
      );
      setEngineer(engs2);
  ...
  if (!enquiry || !customer) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        Enquiry not found.{" "}
        <Button variant="link" onClick={() => navigate("/admin/enquiries")}>
          Go back
        </Button>
      </div>
    );
  ...
  const statusIndex = ALL_STATUSES.indexOf(enquiry.status);
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <CardContent className="p-4 md:p-6">
          ...
          <div className="hidden md:flex items-center justify-between">
            {ALL_STATUSES.map((s, i) => {
              ...
              const historyEntry = enquiry.statusHistory.find(
                (h) => h.status === s,
              );
              return (
                <div
                ...
                >
                  {i > 0 && (
                    <div
                      className={`absolute top-3 right-1/2 w-full h-0.5 -translate-x-0 ${i <= statusIndex ? "bg-primary" : "bg-border"}`}
                    />
                  )}
                  <div
                    className={`relative z-10 w-6 h-6 rounded-full flex items-center justify-center ${reached ? "bg-primary" : "bg-border"}`}
                  >
                    {reached ? (
                  ...
                  <span
                    className={`text-xs mt-2 text-center ${reached ? "font-medium" : "text-muted-foreground"}`}
                  >
                    {s}
                  </span>
                  {historyEntry && (
                    <span className="text-[10px] text-muted-foreground">
                      {format(new Date(historyEntry.timestamp), "dd MMM")}
                    </span>
                  )}
                </div>
              );
              ...
              return (
                <div key={s} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${reached ? "bg-primary" : "bg-border"}`}
                    >
                      {reached ? (
                    ...
                    {i < ALL_STATUSES.length - 1 && (
                      <div
                        className={`w-0.5 h-6 ${reached ? "bg-primary" : "bg-border"}`}
                      />
                    )}
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="material-shadow">
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                {customer.addresses
                  .map(
                    (addr) => `${addr.address1}, ${addr.city}, ${addr.state}`,
                  )
                  .join(" ,")}
              </div>
            </CardContent>
              ...
              <div className="flex gap-2">
                ...
                <div>
                  <span className="font-medium">
                    {enquiry.workTypes.map((wt) => wt.name).join(", ")}
                  </span>{" "}
                  –{" "}
                  {enquiry.workTypes.map((wt) => wt.selectedProduct).join(", ")}
          ...
          <Card className="material-shadow">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                ...
                />
                <Button size="icon" onClick={handleAddRemark}>
        ...
        <div className="space-y-6">
          <Card className="material-shadow">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Assign Engineer</CardTitle>
            </CardHeader>
            <CardContent>
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={engineer?.name || "Select engineer"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {engineersList.map((e) => (
                ...
                <DialogContent>
                  ...
                  <div className="space-y-3">
                    <p className="text-sm">
                      ...
                      <strong>
                        {
                          engineersList.find((e) => e.id === selectedEngineer)
                            ?.name
                        }
                      </strong>
                    </p>
                    {engineerTasks.length > 0 ? (
                      <div>
                        ...
                        {engineerTasks.map((t) => {
                          ...
                          const b = async () => {
                            const a = await CustomerService.getById(
                              t.customer.id,
                            );
                            setTaskCustomers(a);
                          };
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-xs">
                                {format(
                                  new Date(t.siteVisit.scheduledDate),
                                  "dd MMM yyyy",
                                )}{" "}
                                • {t.siteVisit.scheduledTime}
                              </p>
                            </div>
                          );
                        })}
                    ...
                    <div className="flex gap-2 justify-end">
                      <Button
                        variant="outline"
                        onClick={() => {
                          (setAssignDialogOpen(false), setSelectedEngineer(""));
                        }}
                      >
                        Cancel
                      </Button>
