...
const EngineersPage = () => {
  ...
  const getTaskCount = (id: string) =>
    enquiriesList?.filter(
      (e) => e.assignedEngineerId === id && e.status !== "Completed",
    ).length;
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {engineersList?.map((eng) => (
          <Card
            ...
            onClick={() => navigate(`/admin/engineers/${eng.id}`)}
          >
            <CardContent className="p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <Badge
                  variant={eng.status === "Available" ? "default" : "secondary"}
                  className={
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {getTaskCount(String(eng.id))} active tasks
                </div>
              </div>
            </CardContent>
          </Card>
