  ...
  const filtered = useMemo(() => {
    return enquiriesList
      .filter((enq) => {
        ...
        if (
          workTypeFilter !== "all" &&
          !enq.workTypes?.some((wt) => wt.name === workTypeFilter)
        )
          return false;
        if (
          engineerFilter !== "all" &&
          String(enq.assignedEngineerId) !== engineerFilter
        )
          return false;
        if (searchQuery) {
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {dateRange?.from && (
                      <X
                        className="ml-auto h-3 w-3 shrink-0 opacity-50 hover:opacity-100"
                        onClick={(e) => {
                          e.stopPropagation();
      ...
      <div className="space-y-3 md:hidden">
        {filtered.map((enq) => {
          ...
          return (
            <Card
              key={enq.id}
              className="material-shadow cursor-pointer hover:material-shadow-lg transition-shadow"
              onClick={() => navigate(`/admin/enquiries/${enq.id}`)}
            >
              <CardContent className="p-4 space-y-2">
                <div className="flex justify-between items-start">
                  <span className="font-medium">{cust?.name}</span>
                  <Badge variant="outline" className={statusColors[enq.status]}>
                    {enq.status}
                  </Badge>
                </div>
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  <p>
                    ...
                    {enq.workTypes.map((wt) => wt.selectedProduct).join(",")}
                  </p>
