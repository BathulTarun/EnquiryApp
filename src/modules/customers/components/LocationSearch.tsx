...
const LocationSearch = ({onSelect}: LocationSearchProps) => {
  ...
  const getStateId = (stateName: string) => {
    ...
    return states.find((s) => normalize(s.Name) === normalize(stateName));
  };
  ...
  const handleSelectSuggestion = async (loc: Address) => {
    ...
    const details = await LocationService.getPlaceDetails(loc.placeId || "");
              ...
              >
                ...
                <div>
                  ...
                  <span className="block text-xs text-muted-foreground">
                    {loc.address1}, {loc.city} - {loc.pincode}
                  </span>
                </div>
              </button>
            ))}
